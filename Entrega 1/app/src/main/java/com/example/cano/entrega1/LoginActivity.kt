...
class LoginActivity : AppCompatActivity(), LoaderCallbacks<Cursor> {
    ...
    private var mCreateTask: UserCreateask? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        create_account_button.setOnClickListener { attemptCreate() }
    }
    ...
    private fun attemptCreate() {
        ...
        if (cancel) {
            ...
            // perform the user login attempt.
            showProgress(true)
            mCreateTask = UserCreateask(emailStr, passwordStr, this)
            mCreateTask!!.execute(null as Void?)
        }
    }
                ...
                firebaseAuth.signInWithEmailAndPassword(mEmail, mPassword).addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        ...
                        val errorMessage = when (task.exception) {
                            is FirebaseAuthInvalidCredentialsException -> getString(R.string.invalid_email_or_password)
     ...
     * the user.
     */
    inner class UserCreateask internal constructor(private val mEmail: String,
                                                   private val mPassword: String,
                                                   private val context: Context) : AsyncTask<Void, Void, Boolean>() {
            ...
            try {
                ...
                mAuth.createUserWithEmailAndPassword(mEmail, mPassword).addOnCompleteListener { task ->
                    if (task.isSuccessful) {
                        ...
                        Toast.makeText(context, errorMessage, 5*Toast.LENGTH_LONG).show()
                    }
                }
